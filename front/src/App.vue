<template>
  ...
  <div id="app" class="container">
    ...
    <div class="form-group question-body">
      ...
      <div class="input-wrapper"><input id class="form-control question" type="text" ref="q1"
                                        @input="q1=$event.target.value"></div>
      <label>Question2: </label>
      <div class="input-wrapper"><input class="form-control question" ref="q2" v-model="q2" type="text"
                                        @input="q2=$event.target.value"></div>
    </div>
    <button type="button " class="btn btn-primary  btn-block" @click="checkDup">Test</button>
    <div class="alert alert-primary ">
      &nbsp;
      <span v-if="isResult">
        These two questions are {{isDup?'duplicate':'not duplicate'}}, with confidence {{confidence}}%
      </span>
    </div>
  ...
  export default {
    ...
    mounted() {
      ...
      $('.question').typeahead({
        ...
        {
          name: 'states',
          source: substringMatcher(this.qs.map(q => q[1]).concat(this.qs.map(q => q[2])))
        }
      );

      $('.question').bind('typeahead:select', function (ev, suggestion) {
    ...
    data() {
      return {
        ...
        q1: '',
        q2: '',
        qs: [[0, "How does the Surface Pro himself 4 compare with iPad Pro?", "Why did Microsoft choose core m3 and not core i3 home Surface Pro 4?"],
    ...
    methods: {
      checkDup: async function () {
        let that = this;
        let q = this.qs.find(q => q[1] === this.q1);
        if (!q) {
            q = this.qs[Math.round(Math.random() * this.qs.length)]
        }
        console.log(q);
        let {data} = await axios.get(`http://localhost:3802/question/${escape(q[0])}/${escape(q[1])}/${escape(q[2])}`);
        data.result = Math.round(Number(data.result) * 100000) / 1000

        if (Number(data.result) > 50) {
          that.isResult = true;
          that.isDup = true;
          this.confidence = data.result
        } else {
          that.isResult = true;
          that.isDup = false;
          this.confidence = 100 - Number(data.result)
        }
