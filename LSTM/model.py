...
NUM_FOLDS = 10
BATCH_SIZE = 1025
EMBEDDING_FILE = "drive/Quora_Questions_Pairs/glove.840B.300d.txt"
...
def extract_features(df):
    ...
    for i, (q1, q2) in enumerate(list(zip(df["question1"], df["question2"]))):
        ...
        if((q1.split()[0].lower()==q2.split()[0]).lower()):
            features[i, 4] = 1
        else: 
            features[i,4] = 0
    return q1s, q2s, features

train = pd.read_csv("drive/Quora_Questions_Pairs/train.csv")
test = pd.read_csv("drive/Quora_Questions_Pairs/test.csv")
...
train_nlp_features = pd.read_csv("drive/Quora_Questions_Pairs/nlp_features_train.csv")
train_non_nlp_features = pd.read_csv("drive/Quora_Questions_Pairs/non_nlp_features_train.csv")
...
test_nlp_features = pd.read_csv("drive/Quora_Questions_Pairs/nlp_features_test.csv")
test_non_nlp_features = pd.read_csv("drive/Quora_Questions_Pairs/non_nlp_features_test.csv")
...
for idx_train, idx_val in skf.split(train["is_duplicate"], train["is_duplicate"]):
    ...
    submission.to_csv("drive/Quora_Questions_Pairs/data1/preds" + str(model_count) + ".csv", index=False)
