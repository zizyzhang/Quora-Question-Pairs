...
MAX_SEQUENCE_LENGTH = 30        # sentence after position 30 will be ignored
MIN_WORD_OCCURRENCE = 150
...
def get_embedding():
    ...
    for line in f:
        values = line.split()
        word = values[0]
        if len(values) == EMBEDDING_DIM + 1 and word in top_words:  # only words in top words will be collected
...
def prepare(q):
    ...
    new_memento = True
    for w in q.split()[::-1]:
        if w in top_words:                      # if word is top word
            new_q = [w] + new_q                 # top word array
            new_memento = True                  # w is in top words
        elif w not in STOP_WORDS:               # not stop words
            if new_memento:
                new_q = ["memento"] + new_q     # memento added to top word array
                new_memento = False
            if is_numeric(w):
                numbers_q = [w] + numbers_q     # number array
            else:
                surplus_q = [w] + surplus_q     # surplus means word are not top words or number or stop word
        else:
            new_memento = True
...
data_2 = pad_sequences(tokenizer.py(q2s_train), maxlen=MAX_SEQUENCE_LENGTH)
