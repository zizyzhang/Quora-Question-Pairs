...
Tests = [(0, "How does the Surface Pro himself 4 compare with iPad Pro?",
          "Why did Microsoft choose core m3 and not core i3 home Surface Pro 4?")]
with open('model_trained.pkl', 'rb') as f:
    nb_words, embedding_matrix, features_test, _, __ = pickle.load(f)
...
# test = pd.DataFrame({"test_id": [0], "question1": ["How does the Surface Pro himself 4 compare with iPad Pro?"],
...
# features_input = Input(dtype="float32")
features_dense = BatchNormalization()(features_input)
features_dense = Dense(200, activation="relu")(features_dense)
features_dense = Dropout(0.2)(features_dense)
...
model.compile(loss="binary_crossentropy",
              optimizer="nadam")
early_stopping = EarlyStopping(monitor="val_loss", patience=5)
best_model_path = "best_model" + str('9') + ".h5"
model_checkpoint = ModelCheckpoint(best_model_path, save_best_only=True, save_weights_only=True)

model.load_weights("best_model9.h5")
preds = model.predict([test_data_1, test_data_2, features_test], batch_size=BATCH_SIZE, verbose=1);
submission = pd.DataFrame({"test_id": test["test_id"], "is_duplicate": preds.ravel()})
submission.to_csv("predictions/preds_*" + ".csv", index=False)
