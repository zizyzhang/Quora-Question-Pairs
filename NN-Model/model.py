...
def extract_features(df):
    ...
    for i, (q1, q2) in enumerate(list(zip(df["question1"], df["question2"]))):
        ...
        features[i, 0] = len(surplus1.intersection(surplus2))  # 非数字交集长度
        features[i, 1] = len(surplus1.union(surplus2))  # 非数字并集长度
        features[i, 2] = len(numbers1.intersection(numbers2))  # 数字交集长度
        features[i, 3] = len(numbers1.union(numbers2))  # 数字交集长度
...
# df = pd.DataFrame({"id":['0'],"qid1":['1'],"qid2":['2'],"question1":["What is the step by step guide to invest in share market in india?"],"question2":["What is the step by step guide to invest in share market?"],"is_duplicate":['0']})
...
features_input = Input(shape=(25,), dtype="float32")
...
# df_pre = pd.read_csv('sample-2')
# df_pre["question1"] = df_pre["question1"].fillna("").apply(preprocess)
# df_pre["question2"] = df_pre["question2"].fillna("").apply(preprocess)
# q1s_test, q2s_test, test_q_features = extract_features(df_pre)
# test_data_1 = pad_sequences(tokenizer.texts_to_sequences(q1s_test), maxlen=MAX_SEQUENCE_LENGTH)
# test_data_2 = pad_sequences(tokenizer.texts_to_sequences(q2s_test), maxlen=MAX_SEQUENCE_LENGTH)
...
# submission.to_csv("predictions/preds_sample.csv", index=False)

while True:
    try:
        input_qs = input("please input:\n")
        arr = input_qs.split("||||")
        print(input_qs)
        print(arr)
        qid = arr[0]
        ...
        df = pd.DataFrame({"test_id":[qid],"question1":[q1],"question2":[q2]})
        df["question1"] = df["question1"].fillna("").apply(preprocess)
        df["question2"] = df["question2"].fillna("").apply(preprocess)
        q1s_test, q2s_test, test_q_features = extract_features(df)
        nlp_feature_to_pre = test_nlp_features.loc[int(qid):int(qid),:]
        non_nlp_feature_to_pre =test_non_nlp_features.loc[int(qid):int(qid),:]
        test_data_1 = pad_sequences(tokenizer.texts_to_sequences(q1s_test), maxlen=MAX_SEQUENCE_LENGTH)
        test_data_2 = pad_sequences(tokenizer.texts_to_sequences(q2s_test), maxlen=MAX_SEQUENCE_LENGTH)
        features_test = np.hstack((test_q_features, nlp_feature_to_pre, non_nlp_feature_to_pre))
        preds = model.predict([test_data_1, test_data_2, features_test], batch_size=BATCH_SIZE, verbose=1)
        print('$'+str(preds[0][0]))
