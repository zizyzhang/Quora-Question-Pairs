...
def create_question_hash(train_df, test_df_non_nlp):
    train_qs = np.dstack([train_df["question1"], train_df["question2"]]).flatten()
    test_qs = np.dstack([test_df_non_nlp["question1"], test_df_non_nlp["question2"]]).flatten()
...
def get_neighbors(train_df, test_df_non_nlp):
    neighbors = defaultdict(set)
    for df in [train_df, test_df_non_nlp]:
...
train_df = pd.read_csv("data/train-sample.csv")
test_df_non_nlp = pd.read_csv("data/test-sample.csv")

print("Hashing the questions...")
question_dict = create_question_hash(train_df, test_df_non_nlp)
train_df = get_hash(train_df, question_dict)
test_df_non_nlp = get_hash(test_df_non_nlp, question_dict)
print("Number of unique questions:", len(question_dict))

print("Calculating kcore features...")
all_df = pd.concat([train_df, test_df_non_nlp])
kcore_dict = get_kcore_dict(all_df)
train_df = get_kcore_features(train_df, kcore_dict)
test_df_non_nlp = get_kcore_features(test_df_non_nlp, kcore_dict)
train_df = convert_to_minmax(train_df, "kcore")
test_df_non_nlp = convert_to_minmax(test_df_non_nlp, "kcore")

print("Calculating common neighbor features...")
neighbors = get_neighbors(train_df, test_df_non_nlp)
train_df = get_neighbor_features(train_df, neighbors)
test_df_non_nlp = get_neighbor_features(test_df_non_nlp, neighbors)
...
test_df_non_nlp = get_freq_features(test_df_non_nlp, frequency_map)
train_df = convert_to_minmax(train_df, "freq")
test_df_non_nlp = convert_to_minmax(test_df_non_nlp, "freq")

cols = ["min_kcore", "max_kcore", "common_neighbor_count", "common_neighbor_ratio", "min_freq", "max_freq"]
train_df.loc[:, cols].to_csv("data/non_nlp_features_train_sample.csv", index=False)
test_df_non_nlp.loc[:, cols].to_csv("data/non_nlp_features_test_sample.csv", index=False)