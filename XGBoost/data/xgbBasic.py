...
def getFirstSame(row):
    if(len(row['question1']) == 0 or len(row['question2']) == 0):
        return 0
    ques1 = row['question1'][0][0]
    ques2 = row['question2'][0][0]
    if ques1 == ques2:
        return 1
    else:
        return 0

# cwc, csc, ctc features
def get_ques_features(row, stops=None):
    ques_features = [0.0]*6
    safe_div = 0.0001
    ...
    q1_words = set([word for word in row['question1'] if word not in stops])
    q2_words = set([word for word in row['question2'] if word not in stops])
    
    q1_stops = set([word for word in row['question1'] if word in stops])
    q2_stops = set([word for word in row['question2'] if word in stops])
    
    common_word_count = len(q1_words.intersection(q2_words))
    common_stop_count = len(q1_stops.intersection(q2_stops))
    common_token_count = len(set(row['question1']).intersection(set(row['question2'])))
    
    #cwc_min
    ques_features[0] = common_word_count / (min(len(q1_words), len(q2_words)) + safe_div)
    #cwc_max
    ques_features[1] = common_word_count / (max(len(q1_words), len(q2_words)) + safe_div)
    #csc_min
    ques_features[2] = common_stop_count / (min(len(q1_stops), len(q2_stops)) + safe_div)
    #csc_max
    ques_features[3] = common_stop_count / (max(len(q1_stops), len(q2_stops)) + safe_div)
    #ctc_min
    ques_features[4] = common_token_count / (min(len(row['question1']), len(row['question2'])) + safe_div)
    #ctc_max
    ques_features[5] = common_token_count / (max(len(row['question1']), len(row['question2'])) + safe_div)
...
#     X['common_words'] = data.apply(common_words, axis=1, raw=True)  #14
    ...
    f = functools.partial(get_ques_features, stops = stops)
    ques_features = data.apply(f, axis=1, raw=True)
    X['cwc_min'] = list(map(lambda x: x[0], ques_features)) #18
    X['cwc_max'] = list(map(lambda x: x[1], ques_features)) #19
    X['csc_min'] = list(map(lambda x: x[2], ques_features)) #20
    X['csc_max'] = list(map(lambda x: x[3], ques_features)) #21
    X['ctc_min'] = list(map(lambda x: x[4], ques_features)) #22
    X['ctc_max'] = list(map(lambda x: x[5], ques_features)) #23

    X['firstSame'] = data.apply(getFirstSame, axis=1, raw=True) #24
    del ques_features 
...
def main():
    parser = argparse.ArgumentParser(description='XGB Quora')
    name = 'XGB_leaky_add_feature_first'
    parser.add_argument('--save', type=str, default=name,
                        help='save_file_names')
    args = parser.parse_args()

    df_train = pd.read_csv('../data/train_features.csv', encoding="ISO-8859-1")
    X_train_ab = df_train.iloc[:, 2:-1]                             # -1 -> 空
    X_train_ab = X_train_ab.drop('euclidean_distance', axis=1)      #axis=1 对列进行操作
    ...
    # del punctuation
    #df_train['question1'] = df_train.apply(drop_punctuation_q1, axis = 1, raw = True)
    #df_train['question2'] = df_train.apply(drop_punctuation_q2, axis = 1, raw = True)
    ...
    if os.path.isfile('../data/' + args.save + '.mdl'):
        bst = xgb.Booster({'nthread':4})
        bst.load_model('../data/' + args.save + '.mdl')
    else:
